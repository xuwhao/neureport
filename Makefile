NAME=neureport
REPO=xuwhao/neureport
MAIN_ENTRY=cmd/main.go
VERSION=$(shell git describe --tags || echo "unknown")
BUILD=$(shell date +%FT%T%z)
BUILD_DIR=build
RELEASE_DIR=release
GO_BUILD=CGO_ENABLED=0 go build -trimpath -ldflags '-w -s -X "github.com/xuwhao/neureport.Version=${VERSION}" \
		-X "github.com/xuwhao/neureport.Build=${BUILD}" -X "github.com/xuwhao/neureport.Repo=${REPO}"'
